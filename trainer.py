...
import config
import data_manager
from vineyard import correlation_graph_persistence, compute_vine_scores, last_window_centrality
...
def main():
    ...
    for universe_name, tickers in config.UNIVERSES.items():
        ...
        for win in config.WINDOWS:
            ...
            # Compute centrality for this window (using last_day centrality as proxy for topological importance)
            cent = last_window_centrality(returns, win)
            # Map to tickers
            ticker_list = returns.columns.tolist()
            scores = {ticker_list[i]: cent[i] for i in range(len(ticker_list))}
        ...
        print(f"  Top 3 ETFs by eigenvector centrality (topology‑informed): {[e['ticker'] for e in top_etfs]}")
